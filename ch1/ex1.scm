...
;; The following are the `define` statements from this exercise,
;; since the bound values are used in other sections, but you can't
;; bind a name inside of an `and` statement
(define a 3)
(define b (+ a 1))

(or (and (equal? 10 10)
         (write-line "Simple expressions")
         (equal? (+ 5 4 3) 12)
         (equal? (- 9 1) 8)
         (equal? (/ 6 2) 3)

         (write-line "Complex expressions")
         ...
         (write-line "Expressions involving predicates")
         ...
         (equal? (* (cond ((> a b) a)
                    ...
                    (+ a 1))
                 16))
    (error "Expected all expressions to be parsed"))
